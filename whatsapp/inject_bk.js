WAPI.waitNewMessages(false, async (data) => {
    for (let i = 0; i < data.length; i++) {
        ...
        if (intents.blocked.includes(message.chatId.user)) {
        ...
        if (message.type == "chat") {
            ...
            window.log(`typeof de exact`, typeof obj.exact, obj.exact);
            window.log(`typeof de contain`, typeof obj.contains, obj.contains);

            // Verifica se existe mensagem com resposta exata
            var exactMatch = intents.bot.find(obj => obj.exact.find(ex => {

                msgBD  = ex.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();
                msgSRC = message.body.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();

                window.log(`Mensagens EXATA 1`, ex, message.body, message.body.toLowerCase().search(ex));
                window.log(`Mensagens EXATA 2`, msgBD, msgSRC);
            ...
            // Verifica se existe mensagem com resposta parcial
            var PartialMatch = intents.bot.find(obj => obj.contains.find(ex => { 

                msgBD  = ex.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();
                msgSRC = message.body.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase();

                window.log(`Mensagens CONTAIN 1`, ex, message.body, message.body.toLowerCase().search(ex));
                window.log(`Mensagens CONTAIN 2`, msgBD, msgSRC);

                return msgSRC.search(msgBD) > -1
            }));
