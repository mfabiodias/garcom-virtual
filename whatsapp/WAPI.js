...
/*
######################
Adicionadas por Fabio
######################
*/

// Armazena mensagens para envio de campanhas
window.WAPI.campaignData = [];

// Busca mensagens das campanhas para envio.
(async function () {
    while(true) 
    {
        await new Promise(function(resolve){ setTimeout(resolve, (600000)) }); // 10min

        console.log('Buscando dados na API'); 
        
        url = `${messageServerAPI}/api/campaign-messages`;
        await window.WAPI.apiPageData(url, function(err, data) {
            if (err !== null) 
            {
                console.log(`Erro ao buscar dados da campanha: ${err}`); 
            } 
            else 
            {
                if(!!data.length)
                {
                    ...
                    data.forEach(el => {
                        // Adiciona mensagens para envio
                        window.WAPI.campaignData.push(el);

                        // Atuliza status da campanha no servidor
                        url = `${messageServerAPI}/api/campaign-messages-status/${el.id}/freeze`;
                        window.WAPI.apiPageData(url, function(err, data) {
                            if (err !== null) {
                                console.log('Erro ao atualizar status da campanha: ' + err); 
                            } else {
...
// Envia mensagens das campanhas 
(async function () {
    
    const timeRand = [1, 2, 3, 4];

    const randIdx = Math.floor(Math.random() * timeRand.length);

    while(true) 
    {
        await new Promise(function(resolve){ setTimeout(resolve, (timeRand[randIdx] * 1000)) });

        today = new Date();
        checktime = today.getHours() + ":" + today.getMinutes() + ":00";

        console.log(`Horário de checagem: ${checktime}!`);

        window.WAPI.campaignData.forEach((el, idx) => {
            if(el.date <= checktime && !!el.user.length){
                console.log(`Mensagem enviada para ${el.user[0].name}. Msg: ${el.message}`);

                // Envia as mensagens
                window.WAPI.sendSeen(el.user[0].wppid);
                window.WAPI.sendMessage2(el.user[0].wppid, el.message);

                // Envia arquivo se houver
                if (!!el.image) {
                    WAPI.sendImage(el.image_data, el.user[0].wppid, el.image);
                }

                el.user.splice(0, 1);
            }
            else if(!el.user.length){
                console.log(`Campanha ID: ${el.id} concluída com sucesso!`);
                delete window.WAPI.campaignData[idx];

                // Atuliza status da campanha no servidor
                url = `${messageServerAPI}/api/campaign-messages-status/${el.id}/sent`;
                window.WAPI.apiPageData(url, function(err, data) {
                    if (err !== null) {
                        console.log('Erro ao atualizar status da campanha: ' + err);
                    } else {
...
window.WAPI.apiPageData = function(url, callback) {
